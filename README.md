...
+ `models.yaml` рядом с `bootstrap.sh`, скачивает workflows, устанавливает
кастомные ноды через comfy-cli, скачивает модели и перезапускает ComfyUI.
...
URL пишется один раз в переменную и переиспользуется — нужно, чтобы
`bootstrap.sh` узнал свой собственный адрес (`BOOTSTRAP_URL`), извлёк из
него каталог (`BASE_URL`) и подтянул оттуда `fetch_models.py`, `models.yaml`
и содержимое `workflows/`.
...
**Dry-run** — показывает, что скачается, какие ноды установятся, будет ли
рестарт. Ничего не делает.
```bash
...
**Простейший режим без BOOTSTRAP_URL** — используется `DEFAULT_BASE_URL`,
захардкоженный в `bootstrap.sh`.
```bash
...
Распознаются `bootstrap.sh`. Передаются перед `bash` в одной строке.
...
Передаются после `bash -s --`. Прокидываются из `bootstrap.sh` прозрачно.
...
| `--dry-run`                         | Показать план: что качаем, какие ноды ставим, будет ли рестарт. Ничего не делать.                |
...
Коды выхода: `0` — всё ок; `1` — была хотя бы одна неудача.

---

## 4. models.yaml — полная справка

Четыре логические секции в порядке выполнения. Формальная схема:
...
restart: <str>                # optional: auto | none | "<shell command>"
...
Все ключи, относящиеся к нодам, в одной секции — сначала настройки, потом
сам список.
...
|-------------------|--------|---------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| `restart`         | str    | `auto` (дефолт) \| `none` \| `"<shell-команда>"`  | Что делать после установки нод. `auto` — reboot-эндпоинт ComfyUI-Manager (`/api/manager/reboot` → `/manager/reboot`, POST → GET) → фоллбэк `pkill main.py` (подхватит супервизор пода). Срабатывает только если реально что-то установилось и не `--dry-run`. |
| `comfyui_port`    | int    | `8188` (дефолт)                                    | Порт ComfyUI для `restart: auto`.                                                                                              |
| `comfyui_venv`    | path   | пусто (дефолт)                                     | Venv, в котором крутится ComfyUI. **Важно:** туда же ставится pip-пакет `comfyui-manager` и зависимости нод. Без venv используется системный python.       |

#### Список `custom_nodes:`

Плоский список. Элемент — строка (registry-id или git-URL) или словарь.
Правило: строка с `http(s)://` или `git@` → git-URL, иначе → registry-id.
**Оба формата идут через одну и ту же команду** `comfy node install <ref>`,
cm_cli сам разбирается, что ему дали.

```yaml
custom_nodes:
  - comfyui-gguf                                 # registry-id (шорткат)
  - https://github.com/user/Repo                 # git-URL (шорткат)
  - id: comfyui-impact-pack                      # registry-id + опц. пин
    version: 8.15.3                              # best-effort, зависит от Registry
...
|------------|-----------------|---------------------------------------------------------------------------|
| `id`       | registry-форма  | ID ноды в ComfyUI Registry.                                               |
| `version`  | registry-форма  | Опциональный пин версии. Транслируется в `<id>@<version>` для comfy-cli.  |
| `url`      | git-форма       | git-URL репозитория ноды.                                                 |
| `pip`      | обе формы       | `false` — не ставить `requirements.txt` ноды (флаг `--no-deps`). По умолчанию `true`. |

### 4.4. Секция `models:`

Словарь `<type>: [ items ]`, где `<type>` — **буквально имя каталога**
в `<comfyui_path>/models/`. Любой ключ допустим: добавить новый тип
...
- `aria2c` (16 соединений, докачка) — ставится в `bootstrap.sh`.
  Фоллбэк на `curl -C - -L --retry 5`.
- HEAD-проверка размера: если локальный файл совпадает с `Content-Length` —
  пропускается; меньше — докачивается; `--force` игнорирует проверку.
...
- Установщик — `comfy-cli` (ставится в `bootstrap.sh`). Команда одна для
  всех типов спецификации:
  ```
  comfy --skip-prompt --workspace <comfyui> node install <ref>
  ```
  Под капотом она зовёт `cm_cli` из pip-пакета `comfyui-manager`.
- **`comfyui-manager` ставится как pip-пакет автоматически** при первом
  запуске, если его ещё нет. Это требование современного `comfy-cli`
  (>=1.5): он ищет Manager через `import comfyui_manager`, а не как
  директорию `custom_nodes/ComfyUI-Manager/`. Голый git-клон Manager-а
  comfy-cli больше не видит.
- Куда ставится pip-пакет и зависимости нод — определяется ключом
  `comfyui_venv` в YAML. **Важно:** это должно быть то же окружение,
  в котором крутится ComfyUI, иначе ноды не будут импортироваться.
- Идемпотентность: при повторном запуске cm_cli печатает
  `Already exists` + `ERROR`, но возвращает exit code 0, и в наш лог
  попадает `[+] DONE`. Это нормально — нода на месте, run не сорван
  (косметика в TODO в `MEMORY_BANK.md`).

### Рестарт

Срабатывает **в самом конце** (после моделей), один раз, и только если
хотя бы одна нода реально установилась и не `--dry-run`. Стратегия — ключ
`restart`:

- `auto` — POST на `/api/manager/reboot`, при неудаче на `/manager/reboot`
  (затем те же два пути через GET), при полной неудаче `pkill -f ComfyUI/main.py`
  (RunPod-шаблоны обычно перезапускают сервер автоматически).
...
Если на момент завершения скрипта ComfyUI вообще не запущен (типичная
ситуация после первого setup-а пода), все три способа `auto` отработают
вхолостую и в лог попадёт `[!] restart: could not reach Manager API...`.
На работоспособность не влияет — следующий ручной старт ComfyUI подхватит
свежие ноды.
